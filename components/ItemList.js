...
const expandedItemList = ({itemIDs, setFn, isEditable}) => {
  const [items, setItems] = useState(itemIDs);

  useEffect(() => {
    if (itemIDs.length > 0) {
      GETitems(itemIDs).then(setItems)
    }
  }, [itemIDs])
...
const ItemList = ({item, setFn, isEditable=true}) => {
  ...
  return (
    <View style={styles.infoContainer}>
      ...
      <ExpandableView expanded={isItemsExpanded} view={expandedItemList} params={{"itemIDs": item.ids ? item.ids : [], "setFn": setFn, "isEditable": isEditable}} vh={100} />
    </View>
  )
};
